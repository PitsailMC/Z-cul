...
import pitsail.zcul.api.impl.ICullingShader;
import pitsail.zcul.instanced.EntityCullingInstanceRenderer;
import pitsail.zcul.mixin.AccessorFrustum;

import java.util.ArrayList;
import java.util.List;

import static pitsail.zcul.gui.ConfigScreen.u;
import static pitsail.zcul.gui.ConfigScreen.v;
...
public class CullingRenderEvent {
    ...
    public void onOverlayRender(RenderGuiOverlayEvent event) {
        ...
        if (CullingStateManager.DEBUG > 0 && event.getOverlay().id().equals(VanillaGuiOverlay.HELMET.id())) {
            ...
            String cull = Component.translatable("zcul.cull_entity").getString() + ": "
                    + (Config.getCullEntity() ? Component.translatable("zcul.enable").getString() : Component.translatable("zcul.disable").getString());
            addString(monitorTexts, cull);

            String cull_block_entity = Component.translatable("zcul.cull_block_entity").getString() + ": "
                    + (Config.getCullBlockEntity() ? Component.translatable("zcul.enable").getString() : Component.translatable("zcul.disable").getString());
            addString(monitorTexts, cull_block_entity);

            String cull_chunk = Component.translatable("zcul.cull_chunk").getString() + ": "
                    + (Config.getCullChunk() ? Component.translatable("zcul.enable").getString() : Component.translatable("zcul.disable").getString());
            addString(monitorTexts, cull_chunk);

            if (CullingStateManager.DEBUG > 1) {
                String Sampler = Component.translatable("zcul.sampler").getString() + ": " + String.valueOf((Float.parseFloat(String.format("%.0f", Config.getSampling() * 100.0D))) + "%");
                addString(monitorTexts, Sampler);

                if (Config.doEntityCulling()) {
                    String blockCullingTime = Component.translatable("zcul.block_culling_time").getString() + ": " + (CullingStateManager.blockCullingTime / 1000 / CullingStateManager.fps) + " μs";
                    addString(monitorTexts, blockCullingTime);

                    String blockCulling = Component.translatable("zcul.block_culling").getString() + ": " + CullingStateManager.blockCulling + " / " + CullingStateManager.blockCount;
                    addString(monitorTexts, blockCulling);

                    String entityCullingTime = Component.translatable("zcul.entity_culling_time").getString() + ": " + (CullingStateManager.entityCullingTime / 1000 / CullingStateManager.fps) + " μs";
                    addString(monitorTexts, entityCullingTime);

                    String entityCulling = Component.translatable("zcul.entity_culling").getString() + ": " + CullingStateManager.entityCulling + " / " + CullingStateManager.entityCount;
                    addString(monitorTexts, entityCulling);

                    String initTime = Component.translatable("zcul.entity_culling_init").getString() + ": " + (CullingStateManager.entityCullingInitTime / CullingStateManager.cullingInitCount / CullingStateManager.fps) + " ns";
                    addString(monitorTexts, initTime);
                }

                if (Config.getCullChunk()) {
                    if (CullingStateManager.CHUNK_CULLING_MAP != null) {
                        String chunkCullingCount = Component.translatable("zcul.chunk_update_count").getString() + ": " + CullingStateManager.CHUNK_CULLING_MAP.lastQueueUpdateCount;
                        addString(monitorTexts, chunkCullingCount);
                    }

                    String cullingInitTime = Component.translatable("zcul.chunk_culling_init").getString() + ": " + (CullingStateManager.chunkCullingInitTime / CullingStateManager.cullingInitCount / CullingStateManager.fps) + " ns";
